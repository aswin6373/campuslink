...
Future<void> _loadEventData() async {
  ...
  try {
    final responseJson = await ApiClient.get('/api/events/detail/${widget.eventId}');
    if (!mounted) return;
    ...
    setState(() {
      ...
      _locationDetailController.text = event['location_detail']?.toString() ?? '';

      if (event['event_date'] != null) {
        _selectedDate = DateTime.parse(event['event_date'].toString());
   ...
   Future<void> _saveEvent() async {
    ...
    try {
      if (widget.eventId == null) {
        await ApiClient.post('/api/events', body: eventData);
      } else {
        await ApiClient.put('/api/events/${widget.eventId}', body: eventData);
      }
